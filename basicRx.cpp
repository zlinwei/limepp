...
int main(int argc, char **argv) {
    ...
    if (LMS_SetLOFrequency(device, LMS_CH_RX, 0, 91.88e6) != 0)
        error();

    //Set sample rate to 8 MHz, ask to use 2x oversampling in RF
    //This set sampling rate for all channels
    if (LMS_SetSampleRate(device, 4e6, 2) != 0)
        error();
...
#endif
    std::ofstream outfile("rec.dat", std::ios::binary);
    ...
    while (chrono::high_resolution_clock::now() - t1 < chrono::seconds(2)) //run for 5 seconds
    {
        ...
        */
        outfile.write((char *) buffer, samplesRead * 4);
